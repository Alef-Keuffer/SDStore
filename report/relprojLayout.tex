...
\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
  ...
  \node[state] (C1) {Client$_1$};
  \node[state] (C2) [right=of C1]{Client$_2$};
  \node[state] (Cn) [right=of C2]{Client$_n$};
  \node[state] (GlobFifo) [below=of C2]{Global FIFO};
  \node[state] (Srv) [below=of GlobFifo]{Server};
  ...
  \path[->]
    ...
    (GlobFifo) edge node {$read()$} (Srv)
    (M1) edge node {$write(t_1)$} (GlobFifo)
    (M2) edge node {$write(t_2)$} (GlobFifo)
    (Mn) edge node {$write(t_n)$} (GlobFifo)
